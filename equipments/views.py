...
def index_equipments(request):    
    ...
    equipments_count_operational = Equipment.objects.filter(status="success").count()
    equipments_count_partial_dead = Equipment.objects.filter(status="warning").count()
    equipments_count_dead = Equipment.objects.filter(status="danger").count()
    context = {
